import { useEffect, useState, useRef } from 'react'

interface SuggestionCard {
  card_id: string
  name: string
  inclusion: number
  synergy: number
  num_decks: number
  image_uri: string | null
}

interface Category {
  name: string
  cards: SuggestionCard[]
}

interface SuggestionsPanelProps {
  commanderName: string
  deckCardNames: Set<string>
  sections: string[]
  onAdd: (cardId: string, section: string) => Promise<void>
  onClose: () => void
}

export function SuggestionsPanel({ commanderName, deckCardNames, sections, onAdd, onClose }: SuggestionsPanelProps) {
  const [categories, setCategories] = useState<Category[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [adding, setAdding] = useState<Set<string>>(new Set())
  const [sectionMenu, setSectionMenu] = useState<string | null>(null)
  const [hoveredCard, setHoveredCard] = useState<SuggestionCard | null>(null)
  const sectionMenuRef = useRef<HTMLDivElement>(null)
  const scrollRef = useRef<HTMLDivElement>(null)
  const categoryRefs = useRef<Map<string, HTMLDivElement>>(new Map())

  const scrollToCategory = (name: string) => {
    const el = categoryRefs.current.get(name)
    if (el && scrollRef.current) {
      const top = el.offsetTop - scrollRef.current.offsetTop
      scrollRef.current.scrollTo({ top, behavior: 'smooth' })
    }
  }

  useEffect(() => {
    setLoading(true)
    setError(null)
    fetch(`/api/suggestions/edhrec?commander=${encodeURIComponent(commanderName)}`)
      .then(async (res) => {
        if (!res.ok) {
          const data = await res.json().catch(() => ({}))
          throw new Error(data.error ?? `Failed (${res.status})`)
        }
        return res.json()
      })
      .then((data) => setCategories(data.categories ?? []))
      .catch((err) => setError(err.message))
      .finally(() => setLoading(false))
  }, [commanderName])

  // Close section menu on outside click
  useEffect(() => {
    if (!sectionMenu) return
    const handleClick = (e: MouseEvent) => {
      if (sectionMenuRef.current && !sectionMenuRef.current.contains(e.target as Node)) {
        setSectionMenu(null)
      }
    }
    document.addEventListener('mousedown', handleClick)
    return () => document.removeEventListener('mousedown', handleClick)
  }, [sectionMenu])

  const handleAdd = async (card: SuggestionCard, section: string) => {
    setSectionMenu(null)
    setAdding((prev) => new Set(prev).add(card.card_id))
    try {
      await onAdd(card.card_id, section)
    } finally {
      setAdding((prev) => {
        const next = new Set(prev)
        next.delete(card.card_id)
        return next
      })
    }
  }

  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="flex gap-6 items-start max-h-[80vh]">
        {/* Card preview */}
        <div className="w-[300px] shrink-0 hidden lg:block">
          {hoveredCard?.image_uri ? (
            <img
              src={hoveredCard.image_uri}
              alt={hoveredCard.name}
              className="w-full rounded-lg shadow-xl"
            />
          ) : (
            <div className="w-full aspect-[2.5/3.5] bg-gray-800 border border-gray-700 rounded-lg flex items-center justify-center text-sm text-gray-600 p-4 text-center">
              Hover over a card to preview
            </div>
          )}
        </div>

        {/* Main panel */}
        <div className="bg-gray-800 border border-gray-700 rounded-lg w-full max-w-3xl max-h-[80vh] flex flex-col">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700">
            <div>
              <h2 className="text-lg font-semibold">EDHREC Suggestions</h2>
              <p className="text-gray-400 text-sm">{commanderName}</p>
            </div>
            <button onClick={onClose} className="text-gray-400 hover:text-white text-xl">&times;</button>
          </div>

          {!loading && categories.length > 0 && (
            <div className="flex flex-wrap gap-1.5 px-6 py-3 border-b border-gray-700">
              {categories.map((cat) => (
                <button
                  key={cat.name}
                  onClick={() => scrollToCategory(cat.name)}
                  className="px-2.5 py-1 bg-gray-700 hover:bg-gray-600 rounded text-xs text-gray-300 hover:text-white"
                >
                  {cat.name}
                </button>
              ))}
            </div>
          )}

          <div ref={scrollRef} className="flex-1 overflow-y-auto px-6 py-4">
            {loading && <p className="text-gray-400">Loading suggestions...</p>}
            {error && <p className="text-red-400">{error}</p>}
            {!loading && !error && categories.length === 0 && (
              <p className="text-gray-400">No suggestions found for this commander.</p>
            )}

            {categories.map((cat) => (
              <div key={cat.name} ref={(el) => { if (el) categoryRefs.current.set(cat.name, el) }} className="mb-6">
                <h3 className="text-sm font-semibold text-gray-300 mb-2">{cat.name}</h3>
                <div className="space-y-1">
                  {cat.cards.map((card) => {
                    const inDeck = deckCardNames.has(card.name.toLowerCase())
                    return (
                      <div
                        key={card.card_id}
                        className={`flex items-center gap-3 px-3 py-1.5 rounded hover:bg-gray-700/50 ${inDeck ? 'opacity-50' : ''}`}
                        onMouseEnter={() => setHoveredCard(card)}
                        onMouseLeave={() => setHoveredCard(null)}
                      >
                        {card.image_uri && (
                          <img src={card.image_uri} alt="" className="w-8 h-8 rounded object-cover" />
                        )}
                        <span className="flex-1 text-sm">{card.name}</span>
                        <span className="text-xs text-gray-400" title="Inclusion rate">
                          {Math.round(card.inclusion * 100)}%
                        </span>
                        <span className={`text-xs ${card.synergy >= 0 ? 'text-green-400' : 'text-red-400'}`} title="Synergy">
                          {card.synergy >= 0 ? '+' : ''}{Math.round(card.synergy * 100)}%
                        </span>
                        {!inDeck && (
                          <div className="relative">
                            <button
                              onClick={() => setSectionMenu(sectionMenu === card.card_id ? null : card.card_id)}
                              disabled={adding.has(card.card_id)}
                              className="px-2 py-0.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 rounded text-xs font-medium"
                            >
                              +
                            </button>
                            {sectionMenu === card.card_id && (
                              <div ref={sectionMenuRef} className="absolute right-0 top-full mt-1 w-40 bg-gray-700 border border-gray-600 rounded shadow-lg z-50">
                                {sections.map((s) => (
                                  <button
                                    key={s}
                                    onClick={() => handleAdd(card, s)}
                                    className="w-full text-left px-3 py-1.5 text-sm text-gray-200 hover:bg-gray-600 first:rounded-t last:rounded-b"
                                  >
                                    {s}
                                  </button>
                                ))}
                              </div>
                            )}
                          </div>
                        )}
                        {inDeck && (
                          <span className="text-xs text-gray-500">In deck</span>
                        )}
                      </div>
                    )
                  })}
                </div>
              </div>
            ))}
          </div>
        </div>
      </div>
    </div>
  )
}
